...
const CreateNewWork: React.FC<{
  workBody: string;
  changeWorkBody: Dispatch<SetStateAction<string>>;
  changeIsInPreview: Dispatch<SetStateAction<boolean>>;
}> = ({ changeWorkBody, workBody, changeIsInPreview }) => {
  return (
    <>
      <div className="w-full">
        <div className="mb-6 flex w-full flex-col gap-3 rounded-xl border-b-2 border-gray-900 p-0 xs:p-3">
          ...
          <textarea
            value={workBody}
            onChange={(e) => changeWorkBody(e.target.value)}
            id="large-input"
            ...
            <button
              ...
              onClick={() => {
                ...
                if (!parsedWork.success)
                  return parsedWork.error.errors.forEach((err) =>
